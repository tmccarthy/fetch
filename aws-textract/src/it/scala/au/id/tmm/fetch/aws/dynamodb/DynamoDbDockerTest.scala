...
import au.id.tmm.fetch.aws.dynamodb.DynamoDbDockerTest.dynamoInstanceUri
...
import com.github.dockerjava.api.model.{ExposedPort, HealthCheck, HostConfig, PortBinding, Ports}
import com.github.dockerjava.core.{DefaultDockerClientConfig, DockerClientImpl}
import com.github.dockerjava.httpclient5.ApacheDockerHttpClient
import fs2.io.IOException
import munit.CatsEffectSuite
import org.slf4j.{Logger, LoggerFactory}
import software.amazon.awssdk.services.dynamodb.model.{ListTablesRequest, QueryRequest}
  ...
  private val docker: Resource[IO, DockerClient] = Resource.fromAutoCloseable(
    for {
      ...
      _ <- RetryEffect
        ...
        .adaptErr {
          case e => new IOException("Docker isn't running", e)
        }
    } yield client
  )

  private val freePort: IO[Int] = Resource.make(IO(new ServerSocket(0)))(s => IO(s.close()))
  ...
  private def createdDockerContainer(client: DockerClient): Resource[IO, (Port, ContainerId)] = {
    ...
    Resource.make[IO, (Port, ContainerId)](
      for {
        ...
        portToUse <- freePort
        response <- IO {
          client.createContainerCmd("docker.io/amazon/dynamodb-local")
